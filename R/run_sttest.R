...
run_sttest <- function() {
    ...
    for (i in rownames(stats$STTest.Pairs)) {
        ...
        if (stats$STTest.Pairs$ftest[[i]][[1]]$p.value > 0.05) {
            ...
            warning(sprintf("** CHECK DATA - variances between group1 %s & group2 %s ARE NOT homogenous with a p-value: %s (file: %s)", stats$STTest.Pairs[i,"g1"], stats$STTest.Pairs[i,"g2"], stats$STTest.Pairs$ftest[[i]][[1]]$p.value, the$Location.File))
        }
        variance = TRUE
        ...
        stats$STTest.Pairs[i,]$sttest[[1]] = stats::t.test(
            ...
            paired = paired,
            conf.level=0.95)
        message(sprintf("ran a %s students t-test on group1 %s to group2 %s, with tail: %s, variance: %s, p-value: %s", toupper(stats$STTest.Pairs[i,"pair"]), stats$STTest.Pairs[i,"g1"], stats$STTest.Pairs[i,"g2"], toupper(stats$STTest.Pairs[i,"alt"]), toupper(stats$STTest.Pairs[i,"var"]), stats$STTest.Pairs$sttest[[i]]$p.value))
    } # for (i in rownames(Stats.STTest.Pairs)) {
