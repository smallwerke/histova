...
load_data <- function () {
    ...
    raw = utils::read.table(fullPath, sep="\t", header=TRUE, comment.char = '#', check.names = FALSE)
    ...
    raw$Group1 = factor(convert_text(raw$Group1), levels = unique(convert_text(raw$Group1)))
    if (!methods::is(raw$Group2, "NULL")) {
        raw$Group2 = factor(convert_text(raw$Group2), levels = unique(convert_text(raw$Group2)))
    }
    ...
    raw$base <- raw ### CHANGED - using $base as location... ####
